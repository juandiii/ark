...
class MutinyDispatchersTest {
    ...
    void givenGet_thenDispatches() {
        ...
        RequestDispatcher dispatcher = MutinyDispatchers.mutiny(ark);
    ...
    void givenPost_thenDispatches() {
        ...
        assertSame(request, MutinyDispatchers.mutiny(ark).dispatch("POST", "/path"));
    }

    @Test
    void givenPut_thenDispatches() {
        when(ark.put("/path")).thenReturn(request);
        assertSame(request, MutinyDispatchers.mutiny(ark).dispatch("PUT", "/path"));
    }

    @Test
    void givenPatch_thenDispatches() {
        when(ark.patch("/path")).thenReturn(request);
        assertSame(request, MutinyDispatchers.mutiny(ark).dispatch("PATCH", "/path"));
    }

    @Test
    void givenDelete_thenDispatches() {
        when(ark.delete("/path")).thenReturn(request);
        assertSame(request, MutinyDispatchers.mutiny(ark).dispatch("DELETE", "/path"));
    }

    @Test
    void givenUnsupported_thenThrows() {
        assertThrows(ArkException.class, () ->
                MutinyDispatchers.mutiny(ark).dispatch("OPTIONS", "/path"));
    }
}
